...
def solution(N):
    """
    Returns longest binary gap for a positive integer N
    Will return 0 if N doesn't contain a binary gap.
    """
    ...
    while bin_N.endswith('0') and len(bin_N) > 1:
        bin_N = bin_N[:-1]

    longest_binary_gap = 0
    cur_binary_gap = 0
    for element in bin_N:
        if element == '0':
            cur_binary_gap += 1
            longest_binary_gap = max(cur_binary_gap, longest_binary_gap)
        else:
            cur_binary_gap = 0
