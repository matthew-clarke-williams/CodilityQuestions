...
def solution(A, B):
    fish_downstream = []  # initialise list to track fish moving downstream
    alive_fish = 0    # initialise the count for the number of alive fish moving upstream

    for size, direction in zip(A, B):
        if direction == UPSTREAM:
            # Fish moving upstream
            while fish_downstream and fish_downstream[-1] < size:
                # A fish moving downstream meets a fish moving upstream
                fish_downstream.pop()   # Remove smaller downstream fish
            if not fish_downstream:
                # No fish moving downstream so the fish remains alive
                alive_fish += 1
        else:
            # Fish moving downstream
            fish_downstream.append(size)

    return alive_fish + len(fish_downstream)
