...
If a single share was bought on day P and sold on day Q, where 0 ≤ P ≤ Q < N, then the profit of such transaction is
...
days, returns the maximum possible profit from one transaction during this period. The function should return 0 if it
...
def solution(A):
    ...
    Optimal solution time complexity should be O(N)

    Returns maximum profit. Will return 0 if there was no possible max profit slice
    """
    # Initialize the maximum profit to 0
    max_profit = 0

    # If the array is empty or has only one element, return 0 as there can't be any profit
    if len(A) < 2:
        return max_profit
    ...
    for i in range(1, len(A)):
        ...
        # Update the maximum profit if the profit calculated is greater than the current maximum profit
...
print(solution(A))  # Output should be 356
