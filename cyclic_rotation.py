...
def solution(A, K):
    # First, if the array is empty, return the same array
    len_A = len(A)
    if len_A == 0:
        return A

    # Now get the mod of K since we don't need to rotate more than the length of A
    K = K % len_A
    ...
    for i in range(len_A):
        # Need the new index position
        new_index = (i + K) % len_A
        A_rotated[new_index] = A[i]
